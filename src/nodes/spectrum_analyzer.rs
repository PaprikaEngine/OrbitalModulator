...
impl Parameterizable for SpectrumAnalyzerNode {
    define_parameters! {
...
mod tests {
    ...
    fn test_spectrum_analyzer_creation() {
        let analyzer = SpectrumAnalyzerNode::new(44100.0, "test_analyzer".to_string());
    ...
    fn test_spectrum_analyzer_parameters() {
        let mut analyzer = SpectrumAnalyzerNode::new(44100.0, "test".to_string());
    ...
    fn test_bit_reversed_indices() {
        let indices = SpectrumAnalyzerNode::calculate_bit_reversed_indices(8);
        assert_eq!(indices, vec![0, 4, 2, 6, 1, 5, 3, 7]);
    }

    #[test]
    fn test_frequency_bins() {
        let bins = SpectrumAnalyzerNode::calculate_frequency_bins(1024, 44100.0);
